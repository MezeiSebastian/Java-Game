...
public class Game {
    ...
    private void checkForCollisions() {
        ...
        for (Pipe pipe : pipes) {
            if (pipe.collides(bird.positionX, bird.positionY, bird.width, bird.height)) {
                gameover = true;
                bird.isDead = true;
            } else if (pipe.x == bird.positionX && pipe.orientation.equalsIgnoreCase("south")) {
                score++;
            }
        }

        // Ground + Bird collision
        if (bird.positionY + bird.height > App.HEIGHT - 80) {
            gameover = true;
            bird.positionY = App.HEIGHT - 80 - bird.height;
        }
    }
}
